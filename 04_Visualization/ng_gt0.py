...
#raw key
raw_np_array = np.empty(shape=shape, dtype=np.uint8)
for i in range(n_batch):
    raw = daisy.open_ds(f, f'raw/{i}')
    #print("Raw shape: " + str(raw.shape))
    #print("Raw  np array shape: " + str(raw_np_array.shape))
    raw_np_array[i] = raw.to_ndarray().astype(np.float32)/(64*1024)*255
#raw_np_array = np.transpose(raw_np_array, (1, 0, 2, 3))
raw = daisy.Array(data=raw_np_array[0],
                   roi=daisy.Roi((0, 0, 0), roi),
                   voxel_size=(1, 1, 1))

#gt key
gt = daisy.open_ds(f, 'gt/1')
gt_shape = (n_batch, y, x)
gts_np_array = np.empty(shape=gt_shape, dtype=np.uint16)
for i in range(n_batch):
